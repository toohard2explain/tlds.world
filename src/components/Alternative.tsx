...
export default class Alternative extends React.Component<AlternativeProps> {
    ...
    componentDidMount(): void {
        fetch('/api/collect/best').then((response) => {
            ...
            data.forEach((item) => {
                ...
                provider.innerHTML = 'at <a class="underline" href="https://' + item.registrarWebsite + '">' + item.registrar + '</a>';
                ...
                fetch('/api/useable/' + domainString).then((response) => {
                    return response.json();
                }).then((json) => {
                    if (json.useable) {
    ...
    render()  {
        return (
            <div>
                <div
                    className="bg-gray-100 border border-gray-300 py-5 px-6 rounded-xl animate__animated animate__fadeInUp"
                >   
                    <div
                        id="skeleton"
